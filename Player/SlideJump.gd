...
func enter_state() -> void:
	...
	# Apply jump speed
	player.velocity += player.slideJumpSpeed * player.slideJumpNormal
...
func process_state(delta):
	...
	if axis.x == sign(player.slideJumpNormal.x):
	...
	if Input.is_action_just_released("jump") && !jumpButtonReleased:
		player.velocity.y = max(player.velocity.y, -player.minSlideJumpSpeed)
